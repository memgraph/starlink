...
def plot_n_orbits(rs, labels, cb=pd.earth, show_plot=False,save_plot=False,title=''):
    ...
    n=0
    for r in rs:
        #ax.plot(r[:, 0], r[:,1], r[:,2], 'w', label=labels[n]) only if there are a few satellites can labels be used
