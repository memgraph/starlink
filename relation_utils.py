from utils import distance3D as d3D
...
def init_orbits_and_objects(num_of_orbits_horizontal, num_of_orbits_vertical, num_of_objects_in_orbit_h, num_of_objects_in_orbit_v, size_x, size_y, moving_object_speed, satellite_altitude):
    ...
    for x in range(num_of_orbits_horizontal):
        orbit = Orbit(id=orbit_id, id_in_orbit_group=x, is_horizontal=True, x_start=-size_x/2, y_start=(x+1)*(size_y/(num_of_orbits_horizontal+1))-90, x_end=size_x/2-1, y_end=(x+1)*(size_y/(num_of_orbits_horizontal+1))-90,
        ...
        for y in range(num_of_objects_in_orbit_h):
            ...
            mo = MO(id=moving_object_id, id_in_orbit=y, orbit_id=orbit_id, is_in_horizontal_orbit=True, x0=y*(size_x/num_of_objects_in_orbit_h)-180,
                    y0=(x+1)*(size_y/(num_of_orbits_horizontal+1))-90, z0=satellite_altitude, laser_left_id=laser_left_id, laser_left_id_in_orbit=laser_left_id_in_orbit, laser_right_id=laser_right_id, laser_right_id_in_orbit=laser_right_id_in_orbit)
    ...
    for x in range(num_of_orbits_vertical):
        orbit = Orbit(id=orbit_id, id_in_orbit_group=x, is_horizontal=False, x_start=(x+1)*(size_x/(num_of_orbits_vertical+1))-180, y_start=-size_y/2, x_end=(x+1)*(size_x/(num_of_orbits_vertical+1))-180, y_end=size_y/2-1,
        ...
        for y in range(num_of_objects_in_orbit_v):
            ...
            mo = MO(
                id=moving_object_id, id_in_orbit=y, orbit_id=orbit_id, is_in_horizontal_orbit=False, x0=(x+1)*(size_x/(num_of_orbits_vertical+1))-180,
                y0=y*(size_y/num_of_objects_in_orbit_v)-90, z0=satellite_altitude, laser_left_id=laser_left_id, laser_left_id_in_orbit=laser_left_id_in_orbit, laser_right_id=laser_right_id, laser_right_id_in_orbit=laser_right_id_in_orbit)
            orbit.moving_objects.append(mo)
            # orbit.moving_objects_dict[moving_object_id] = mo
            all_moving_objects.append(mo)
            all_moving_objects_dict[moving_object_id] = mo
